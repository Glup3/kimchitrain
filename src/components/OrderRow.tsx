...
export default function OrderRow({ order }: { order: OrderRowProps }) {
	...
	const hasUnsettled = order.completed && unsettledCents > 0
	...
	return (
		<Link
			...
			className={cn(
				...
				!allSettled && 'text-[var(--sea-ink)]',
			)}
		>
			<div className="flex flex-col gap-0.5">
				<div className="flex items-center gap-3">
					<span className={cn('text-base font-medium', allSettled && 'text-[var(--sea-ink-soft)] line-through')}>
						Order {order.id.slice(-8)}
					</span>
					{hasUnsettled && (
						<span className="flex items-center gap-1 text-xs font-medium text-[var(--palm)]">
							<AlertCircle size={12} />€{(unsettledCents / 100).toFixed(2)} owed
						</span>
					)}
					{allSettled && (
